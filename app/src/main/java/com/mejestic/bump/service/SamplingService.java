package com.mejestic.bump.service;
...
import com.mejestic.bump.ui.MainActivity;
...
public class SamplingService extends Service
  ...
  private int gravityAcclerationctr;
  private int state;
  private int sensorRate;
  private SensorManager sensorManager;
  private PrintWriter captureFile;
  private Sensor mAccelerometerSensor;
  private boolean mDataAnalysisStarted = false;
  private double sensitivity = 160.0;
  private double gravityAccleration = 0.0;
  ...
  public void onSensorChanged(SensorEvent event) {
    ...
    analyseAccelerometerData(timestamp, x, y, z);
  }

  private void stopDataAnalysis() {
    if (!mDataAnalysisStarted) return;
  ...
  private void analyseData() {
    ...
    if (mAccelerometerSensor != null) {
      ...
      sensorManager.registerListener(this, mAccelerometerSensor, sensorRate);
    }
    captureFile = null;
    if (DEBUG_GENERAL) {
  ...
  private void initSampling() {
    ...
    gravityAccleration = 0.0;
    gravityAcclerationctr = GRAVITYACCEL_MEAS_LEN;
  }

  private void startMeasuring() {
    lpFilt = new FIR(meanCoff);
  ...
  private void analyseAccelerometerData(long timestamp, float x, float y, float z) {
    double amplitude = Math.sqrt((double) x * x +
        (double) y * y +
        (double) z * z);
    switch (state) {
      case STATE_GRAVITYACCEL:
        //Mean amplitude , vector addition of accelerometer amplitude and gravity
        gravityAccleration += amplitude;
        --gravityAcclerationctr;
        if (gravityAcclerationctr <= 0) {
          gravityAccleration /= ((double) GRAVITYACCEL_MEAS_LEN);
          startMeasuring();
        }
        break;

      case STATE_MEASURING:
        amplitude -= gravityAccleration;
        amplitude = lpFilt.filter(amplitude);
        double a0_9 = bw_0_9_filt.filter(amplitude);
        double a1_0 = bw_1_0_filt.filter(amplitude);
        double a1_1 = bw_1_1_filt.filter(amplitude);
        ...
        if ((pw_0_9_state && !pw_0_9_prevstate) ||
          ...
          if ((lastEdge < 0L) || ((sampleCounter - lastEdge) > STABLEWINDOW)) {
            ...
            sendResult(mCurrentLocation.getLatitude() + "," + mCurrentLocation.getLongitude());
            ++bumps;
            Log.d(LOG_TAG, "Bump found");
          }
  ...
  class FIR {
    ...
    public double filter(double input) {
      filterBuffer[bufferPointer] = input;
      int tempPointer = bufferPointer;
      double mac = 0.0;
      for (int i = 0; i < filterLength; ++i) {
        mac = mac + coeffs[i] * filterBuffer[tempPointer];
        --tempPointer;
        if (tempPointer < 0) tempPointer = filterLength - 1;
      }
      bufferPointer = (++bufferPointer) % filterLength;
      return mac;
    }
  }
  ...
  class PowerWindow {
    ...
    public double power(double inp) {
      filterBuffer[bufferPointer] = inp;
      int tmp_ptr = bufferPointer;
      double mac = 0.0;
      for (int i = 0; i < filterLength; ++i) {
        mac = mac + filterBuffer[tmp_ptr] * filterBuffer[tmp_ptr];
        --tmp_ptr;
        if (tmp_ptr < 0) tmp_ptr = filterLength - 1;
      }
      mac = mac / (double) filterLength;
      mac = Math.sqrt(mac);
      bufferPointer = (++bufferPointer) % filterLength;
      return mac;
    }
